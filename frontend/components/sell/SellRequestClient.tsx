...
export default function SellRequestClient({
  title,
  description,
  requestType,
  dashboardKey,
}: SellRequestClientProps) {
  ...
  const summary = useMemo(
    () => `${year || "Year"} ${vehicle || "vehicle"}${price ? ` | Asking ${price}` : ""}`,
    [price, vehicle, year],
  );
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_0.9fr]">
      <form onSubmit={handleSubmit} className="grid gap-4 rounded-3xl border border-black/10 bg-surface p-6">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <label className="grid gap-1 text-sm font-semibold">
            Vehicle
            <input value={vehicle} onChange={(event) => setVehicle(event.target.value)} className="rounded-lg border border-black/15 px-3 py-2 font-normal" placeholder="Make and model" />
          </label>
          <label className="grid gap-1 text-sm font-semibold">
            Year
            <input value={year} onChange={(event) => setYear(event.target.value)} className="rounded-lg border border-black/15 px-3 py-2 font-normal" placeholder="e.g. 2021" />
          </label>
          <label className="grid gap-1 text-sm font-semibold">
            Asking Price
            <input value={price} onChange={(event) => setPrice(event.target.value)} className="rounded-lg border border-black/15 px-3 py-2 font-normal" placeholder="Optional" />
          </label>
        </div>

        <label className="grid gap-1 text-sm font-semibold">
          Notes
          <textarea value={notes} onChange={(event) => setNotes(event.target.value)} rows={4} className="rounded-lg border border-black/15 px-3 py-2 font-normal" placeholder="Condition, swap preferences, service details, etc." />
        </label>
      ...
      <aside className="rounded-3xl border border-black/10 bg-surface p-6">
        ...
        <div className="mt-4 rounded-2xl border border-brand/20 bg-brand-soft/30 p-4 text-sm text-foreground">
          Submitted requests are sent to the team and stored in the browser dashboard history for quick follow-up.
        </div>
      </aside>
    </div>
  );
