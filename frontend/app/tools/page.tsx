...
const toolsGroup = megaMenuGroups.find((group) => group.id === "ownership-tools");
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6">
      <section className="rounded-3xl border border-black/10 bg-surface p-6 sm:p-10">
        ...
        <h1 className="mt-3 text-3xl font-bold sm:text-5xl">Tools Hub</h1>
        <p className="mt-4 max-w-3xl text-base text-ink-muted sm:text-lg">
          Use the tools below to match the right car, plan financing, estimate value, compare options and verify history.
        </p>
      </section>
      ...
      <section className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {toolsGroup?.items.map((item) => (
          <Link key={item.href} href={item.href} className="rounded-2xl border border-black/10 bg-surface p-5 transition hover:border-brand/40 hover:shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-wide text-brand">Tool</p>
